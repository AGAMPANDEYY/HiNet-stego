...
try:
    ...
    for i_epoch in tqdm(range(c.epochs), desc="epochs"):
        ...
        #     train:    #
        #################
        with tqdm(datasets.trainloader, desc=f"Epoch {i_epoch}") as pbar:
         ...
         for i_batch, data in enumerate(pbar):
            ...
            total_loss.backward()
            if (i_batch + 1) % c.gradient_accumulation_steps == 0 or i_batch == len(datasets.trainloader) - 1:
              optim.step()
              optim.zero_grad()
